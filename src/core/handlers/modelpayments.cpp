...
//      const int nSourceRow    = headerData(index.row(),    Qt::Vertical,   Qt::UserRole).toInt();
        ...
        if (PMNT_SOURCE_COL_ROW_WIDGET_ID == nSourceColumn)
        {
...
//    }
        ...
        QSortFilterProxyModel::data(index,role).isValid())
    {
        ...
        if (nSourceColumn == PMNT_SOURCE_COL_ROW_WIDGET_ID)
        {
        ...
        if (nSourceColumn == PMNT_SOURCE_COL_MY_NYM_ID) // my_nym_id
        {
        ...
        else if (nSourceColumn == PMNT_SOURCE_COL_ACCOUNT_ID) // my_acct_id
        {
            QString qstrID = sourceData.isValid() ? sourceData.toString() : "";
            ...
            {
                const std::string str_id = qstrID.trimmed().toStdString();
                str_name = str_id.empty() ? "" : opentxs::OT::App().Client().Exec().GetAccountWallet_Name(str_id);
            }
            // ------------------------
            if (str_name.empty() && !qstrID.isEmpty())
                return QVariant(qstrID);
...
//        }
        else if (nSourceColumn == PMNT_SOURCE_COL_NOTARY_ID) // notary_id
        {
            ...
            const std::string str_name = str_id.empty()
                ? ""
                : opentxs::OT::App().Client().Exec().GetServer_Name(str_id);
            // ------------------------
            if (!str_name.empty())
                return QVariant(QString::fromStdString(str_name));
        ...
        else if (nSourceColumn == PMNT_SOURCE_COL_AMOUNT) // amount
        {
            ...
            QString qstrAmount = QString::fromStdString(opentxs::OT::App().Client().Exec().LongToString(lAmount));

            if (!qstrAssetType.isEmpty())
            {
                QString qstrTemp = QString::fromStdString(opentxs::OT::App().Client().Exec().FormatAmount(qstrAssetType.toStdString(), lAmount));
                if (!qstrTemp.isEmpty())
                    qstrAmount = qstrTemp;
            }
            // ----------------------------------------------
        ...
        else if (nSourceColumn == PMNT_SOURCE_COL_UNIT_TYPE_ID) // my_asset_type_id
        {
            QString qstrID = sourceData.isValid() ? sourceData.toString() : "";
            const std::string str_id = qstrID.toStdString();
            const std::string str_name = str_id.empty() ? "" : opentxs::OT::App().Client().Exec().GetAssetType_Name(str_id);
            // ------------------------
            if (!str_name.empty())
                return QVariant(QString::fromStdString(str_name));
...
//        const QString qstrTxnId            = lTxnId        > 0 ? QString::fromStdString(opentxs::OT::App().Client().Exec().LongToString(lTxnId       )) : "";
//        const QString qstrTxnIdDisplay     = lTxnIdDisplay > 0 ? QString::fromStdString(opentxs::OT::App().Client().Exec().LongToString(lTxnIdDisplay)) : "";
...
//                ? QString("")
//                : QString::fromStdString(opentxs::OT::App().Client().Exec().GetServer_Name(qstrMsgNotaryID.toStdString()));

//            const QString qstrPmntNotaryName = qstrPmntNotaryID.isEmpty()
//                ? QString("")
//                : QString::fromStdString(opentxs::OT::App().Client().Exec().GetServer_Name(qstrPmntNotaryID.toStdString()));

//            const QString qstrMyAcctName = qstrMyAcct.isEmpty()
//                ? QString("")
//                : QString::fromStdString(opentxs::OT::App().Client().Exec().GetAccountWallet_Name(qstrMyAcct.toStdString()));

//            const QString qstrAssetName = qstrAssetType.isEmpty()
//                ? QString("")
//                : QString::fromStdString(opentxs::OT::App().Client().Exec().GetAssetType_Name(qstrAssetType.toStdString()));
...
ActivityPaymentsProxyModel::ActivityPaymentsProxyModel(QObject *parent /*=0*/)
: QSortFilterProxyModel(parent)
{
}

void ActivityPaymentsProxyModel::setFilterFolder(int nFolder)
{
    nFolder_ = nFolder; // 0 for outbox, 1 for inbox, and 2+ for all other future custom boxes.
    invalidateFilter();
}

void ActivityPaymentsProxyModel::setFilterNone()
{
    clearFilterType();
    filterType_ = FilterNone;
    // --------------------------------
...
void ActivityPaymentsProxyModel::setFilterTopLevel(QString qstrContactID)
{
    clearFilterType();
    filterType_ = FilterTopLevel;
    // --------------------------------
...
void ActivityPaymentsProxyModel::setFilterNotary(QString qstrNotaryId)
{
    clearFilterType();
    filterType_ = FilterNotary;
    // --------------------------------
...
void ActivityPaymentsProxyModel::setFilterCurrency(int nCurrencyType)
{
    OT_FAIL;
    clearFilterType();
    filterType_ = FilterCurrency;
    // --------------------------------
//    MTContactHandler::getInstance()->GetAssetIdsForTLA(mapAssetTypes_, qstrTLA.toStdString());
    // --------------------------------
    invalidateFilter();
}

void ActivityPaymentsProxyModel::setFilterAccount(QString qstrAccountId)
{
    clearFilterType();
    filterType_ = FilterAccount;
    // --------------------------------
    accountId_ = qstrAccountId;
    // --------------------------------
    invalidateFilter();
}

void ActivityPaymentsProxyModel::setFilterString(QString qstrFilter)
{
    filterString_ = qstrFilter;
    invalidateFilter();
}

void ActivityPaymentsProxyModel::clearFilterType()
{
    filterType_       = FilterNone;
    // --------------------------------
    notaryId_         = QString("");
    ...
    // --------------------------------
    mapNymIds_.clear();
    mapAssetTypes_.clear();
}


QWidget * ActivityPaymentsProxyModel::CreateDetailHeaderWidget(const int nSourceRow, bool bExternal/*=true*/) const
{
    QModelIndex sourceIndex_RowWidgetId    = sourceModel()->index(nSourceRow, PMNT_SOURCE_COL_ROW_WIDGET_ID);
    QModelIndex sourceIndex_CurrencyType   = sourceModel()->index(nSourceRow, PMNT_SOURCE_COL_CURRENCY_TYPE);
    QModelIndex sourceIndex_MyNymId        = sourceModel()->index(nSourceRow, PMNT_SOURCE_COL_MY_NYM_ID);
...
QVariant ActivityPaymentsProxyModel::headerData(int section, Qt::Orientation orientation, int role/*=Qt::DisplayRole*/) const
{
    QVariant varHeaderData = QSortFilterProxyModel::headerData(section, orientation, role);
...
bool ActivityPaymentsProxyModel::filterAcceptsColumn(int source_column, const QModelIndex & source_parent) const
{
    bool bReturn = true;
...
// Instead, it contains a NymID and/or a Bitmessage address and/or a notary address.
//
// When the user clicks on a top-level Contact ID, we need to filter on ALL known Nyms for that Contact.
// Any single one of them could be the one on the Payment, that causes the filter to approve it.
// Notice in that case they're not ALL required -- just ONE of them.
//
// Next let's say a user clicks on a Notary. In that case we need to filter purely on that Notary.
// Next let's say a user clicks on an Account. In that case we need to filter purely on that Account.
//
// Next let's say a user clicks on a TLA. In that case, we need to filter on ALL known Asset Type IDs for that TLA.
// Any single one of them could be the one on the payment, that causes the filter to approve it.
// Notice in that case they're not ALL required -- just ONE of them.
//
// That means there are 4 main filter types:
// 1. notary ID by itself
// 2. account ID by itself
// 3. TLA resulting in a list of asset type IDs.
// 4. top-level: Contact ID resulting in a list of Nym IDs.
//
bool ActivityPaymentsProxyModel::filterAcceptsRow(int sourceRow, const QModelIndex &sourceParent) const
{
//    QModelIndex indexTxnId        = sourceModel()->index(sourceRow, PMNT_SOURCE_COL_TXN_ID,         sourceParent); // txn_id
//    QModelIndex indexTxnIdDisplay = sourceModel()->index(sourceRow, PMNT_SOURCE_COL_TXN_ID_DISPLAY, sourceParent); // txn_id_display
//    QModelIndex indexMyNym        = sourceModel()->index(sourceRow, PMNT_SOURCE_COL_MY_NYM,         sourceParent); // my_nym_id
...
//                if (   //(0 == qstrNymID.compare(qstrMyNym)) ||
//                       (0 == qstrNymID.compare(qstrSenderNym))
//                    || (0 == qstrNymID.compare(qstrRecipientNym))
//                        ) // Match!
//                    return true;
//            }
//            return false;
...
//        case FilterAccount:
//        {
//            if ( qstrMyAcct == accountId_ ) // The row matches our filter.
//                return true;
////            if ( ((qstrSenderAcct == accountId_) || (qstrRecipientAcct == accountId_)) ) // The row matches our filter.
////                return true;
//            return false;
//        }
//            break;
...
//        case FilterTLA:
//        {
//            for (mapIDName::const_iterator ii = mapAssetTypes_.begin(); ii != mapAssetTypes_.end(); ii++)
//            {
//                const QString qstrAssetTypeID = ii.key();

//                if ( 0 == qstrAssetTypeID.compare(qstrAssetType) ) // Match!
//                    return true;
//            }
//            return false;
//        }
...
//        default:
//            qDebug() << "Unexpected filter type in ActivityPaymentsProxyModel::filterAcceptsRow.\n";
//            return false;
//        }
//    }
...
QVariant ActivityPaymentsProxyModel::rawData ( const QModelIndex & index, int role/* = Qt::DisplayRole */) const
{
    return QSortFilterProxyModel::data(index,role);
}
