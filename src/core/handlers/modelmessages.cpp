...
void ConvMsgsProxyModel::setFilterMethodAddress(QString qstrMethodType, QString qstrAddress)
{
    clearFilterType();
...
//        }
//        else
        if (nSourceColumn == MSG_SOURCE_COL_TIMESTAMP) // timestamp
        {
...
//            bReturn = false;
    ...
    {
        if (nFolder_ == 1) // If the folder is the inbox, hide recipient. (We normally see a sender in the inbox, not a recipient.)
            return false;
        else
            bReturn = true;
    }
    ...
    case MSG_SOURCE_COL_RECIP_ADDR:
    {
        if (FilterNotary == filterType_) // If message was sent through a notary, then it DEFINITELY won't have a Bitmessage address.
            bReturn = false;
        else if (nFolder_ == 1) // Inbox hides recipient.
            return false;
        else
            bReturn = false; // Current expected behavior.
    }
    ...
    case MSG_SOURCE_COL_SUBJECT:         bReturn = false;  break;
