...
void MTCredentials::on_treeWidget_itemSelectionChanged()
{
    ...
    if (bIsNym)
    {
        ...
        // --------------------------------
        const std::string str_source = opentxs::OTAPI_Wrap::Exec()->GetNym_SourceForID(qstrNymID.toStdString());
        // --------------------------------
    ...
    else if (bIsCred)
    {
        ...
        // --------------------------------
        const std::string str_contents = opentxs::OTAPI_Wrap::Exec()->GetNym_MasterCredentialContents(qstrNymID .toStdString(),
    ...
    else if (bIsSubcred)
    {
        ...
        // --------------------------------
        const std::string str_contents = opentxs::OTAPI_Wrap::Exec()->GetNym_ChildCredentialContents(qstrNymID .toStdString(),
                                                                                  qstrCredID.toStdString(),
...
void MTCredentials::refresh(QStringList & qstrlistNymIDs)
{
    ...
    if ((NULL != ui) && (qstrlistNymIDs.size() > 0))
    {
        ...
        for (int ii = 0; ii < m_NymIDs.size(); ++ii)
        {
            ...
            //
            const int32_t nCountCredentials = opentxs::OTAPI_Wrap::Exec()->GetNym_MasterCredentialCount(str_nym_id);

            for (int nCred = 0; nCred < nCountCredentials; ++nCred)
            {
                std::string str_cred_id = opentxs::OTAPI_Wrap::Exec()->GetNym_MasterCredentialID(str_nym_id, nCred);
                ...
                //
                // std::string opentxs::OTAPI_Wrap::Exec()->GetNym_CredentialContents(const std::string & NYM_ID, const std::string & CREDENTIAL_ID);
                // ---------------------------------------
                // Next: any subcredentials under this credential?
                //
                const int32_t nCountSubcred = opentxs::OTAPI_Wrap::Exec()->GetNym_ChildCredentialCount(str_nym_id, str_cred_id);

                for (int nSubcred = 0; nSubcred < nCountSubcred; ++nSubcred)
                {
                    std::string str_sub_cred_id = opentxs::OTAPI_Wrap::Exec()->GetNym_ChildCredentialID(str_nym_id, str_cred_id, nSubcred);
                    ...
                    //
                    // Also useful:
                    // std::string opentxs::OTAPI_Wrap::Exec()->AddSubcredential(const std::string & NYM_ID, const std::string & MASTER_CRED_ID, const int32_t & nKeySize);
                    // bool        opentxs::OTAPI_Wrap::Exec()->RevokeSubcredential(const std::string & NYM_ID, const std::string & MASTER_CRED_ID, const std::string & SUB_CRED_ID);
                    //
                } // for (subcredentials.)
            } // for (credentials.)
...
MTCredentials::NameStatusFunctor::operator() (const QSqlRecord& rec)
{
  ...
  if (!active || unconfirmed)
  ...
  else
    {
      ...
      std::string nymSrc;
      nymSrc = opentxs::OTAPI_Wrap::Exec()->GetNym_SourceForID (nym);
