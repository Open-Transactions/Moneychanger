...
//  mapOfNymClaims nym_claims; // Each pair in this map has a NymID and a ClaimSet.
    ...
    const std::string str_nym_name = Moneychanger::It()->OT().Exec().GetNym_Name(qstrNymId.toStdString());
...
void MTNymDetails::refresh(QString strID, QString strName)
{
    ...
    {
        std::string nym_description = Moneychanger::It()->OT().Exec().GetNym_Description(strID.toStdString());
