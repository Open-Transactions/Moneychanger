...
void MTOfferDetails::AddButtonClicked()
{
    ...
    // ---------------------------------------------------
    QString qstrMarketNymName = QString::fromStdString(Moneychanger::It()->OT().Exec().GetNym_Name(qstrMarketNymID.toStdString()));
    ...
    if (qstrMarketNotaryID.isEmpty() || (0 == qstrAll.compare(qstrMarketNotaryID)))
    {
        ...
        qstrMarketServerName = QString::fromStdString(Moneychanger::It()->OT().Exec().GetServer_Name(qstrMarketNotaryID.toStdString()));
    // ---------------------------------------------------
...
//      const QString qstrCurrencyName       (theWizard.field("CurrencyName")       .toString());
        ...
        const int64_t lTotalAsset            (Moneychanger::It()->OT().Exec().StringToAmount(str_asset_id,           //...and lTotalAsset is 9000
        ...
        const int64_t lScale                 (Moneychanger::It()->OT().Exec().StringToAmount(str_asset_id,                 // ...then lScale is 1000
                                                                         qstrScale  .toStdString   ()));
        // --------------------------------------------
        const QString qstrPrice              (theWizard.field("pricePerScale")      .toString      ());// If this is "$ 45.98"
        const int64_t lPrice                 (Moneychanger::It()->OT().Exec().StringToAmount(str_currency_id,              // ...then lPrice is 4598
        ...
        // --------------------------------------------
        const int64_t lAssetAcctBalance      (Moneychanger::It()->OT().Exec().StringToAmount(str_asset_id,
                                                                         qstrAssetAcctBalance   .toStdString()));
        const int64_t lCurrencyAcctBalance   (Moneychanger::It()->OT().Exec().StringToAmount(str_currency_id,
...
*/
        ...
        else // Limit order
        {
            ...
            {
                const QString qstrNeededBalance = QString::fromStdString(Moneychanger::It()->OT().Exec().FormatAmount(
        ...
        std::string  strResponse;
        {
            ...
            // --------------------------------------------------------------
            auto action = Moneychanger::It()->OT().ServerAction().CreateMarketOffer(opentxs::Identifier::Factory(str_asset_acct_id),
        ...
        if (m_pOwner->m_pmapOffers->end() != it_offer)
        {
            ...
            if (NULL != pOfferData) // Should never be NULL.
            {
                ...
                std::string  strResponse;
                {
                    MTSpinner theSpinner;
                    // --------------------------------------------------------------
                    auto action = Moneychanger::It()->OT().ServerAction().KillMarketOffer(opentxs::Identifier::Factory(str_nym_id),
        ...
        if (m_pOwner && m_pOwner->m_pmapOffers)
        {
            ...
            if (m_pOwner->m_pmapOffers->end() != it_offer)
            {
                ...
                if (NULL != pOfferData) // Should never be NULL.
                {
                    ...
                    // ------------------------------------------------------
                    int64_t     lMinimumIncrement = Moneychanger::It()->OT().Exec().StringToLong(pOfferData->minimum_increment);
                    // ------------------------------------------------------
                    ...
                    // ------------------------------------------------------
                    QString qstrMinimumIncrement  = QString::fromStdString(Moneychanger::It()->OT().Exec().FormatAmount(pOfferData->instrument_definition_id, lMinimumIncrement));
                    ...
                    QString qstrStillAvailable    = QString::fromStdString(Moneychanger::It()->OT().Exec().FormatAmount(pOfferData->instrument_definition_id, lStillAvailable));
                    // ------------------------------------------------------
                    std::string str_asset_name    = Moneychanger::It()->OT().Exec().GetAssetType_Name(pOfferData->instrument_definition_id);
                    // -----------------------------------------------------------------------
                    time_t tValidFrom      = static_cast<time_t>(Moneychanger::It()->OT().Exec().StringToLong(pOfferData->valid_from));
                    time_t tValidTo        = static_cast<time_t>(Moneychanger::It()->OT().Exec().StringToLong(pOfferData->valid_to));
                    // -----------------------------------------------------------------------
...
//                    QString qstrTrans = QString("<font size=1 color=grey>%1#</font> %2").arg(tr("Trans")).arg(qstrTransactionID);
                    ...
                    //
                    // ------------------------------------------------------
                    ui->lineEditAssetAcct      ->setText(QString::fromStdString(Moneychanger::It()->OT().Exec().GetAccountWallet_Name(pOfferData->asset_acct_id)));
                    ui->lineEditCurrencyAcct   ->setText(QString::fromStdString(Moneychanger::It()->OT().Exec().GetAccountWallet_Name(pOfferData->currency_acct_id)));
                    // ------------------------------------------------------
                    ...
                    // ------------------------------------------------------
                    const int64_t lAcctBalance     = Moneychanger::It()->OT().Exec().GetAccountWallet_Balance(pOfferData->asset_acct_id);
                    const int64_t lCurrencyBalance = Moneychanger::It()->OT().Exec().GetAccountWallet_Balance(pOfferData->currency_acct_id);

                    const std::string str_acct_balance     = Moneychanger::It()->OT().Exec().FormatAmount(pOfferData->instrument_definition_id,    lAcctBalance);
                    const std::string str_currency_balance = Moneychanger::It()->OT().Exec().FormatAmount(pOfferData->currency_type_id, lCurrencyBalance);
    ...
    if (OFFER_MAP.end() != it_offer)
    {
        ...
        if (NULL != pOfferData) // Should never be NULL.
        {
            ...
            if (pModel && pTradeDataProxy_)
            {
                ...
                int64_t lOfferID = Moneychanger::It()->OT().Exec().StringToLong(pOfferData->transaction_id);
...
//    this->blockSignals(true);
    ...
    if (OFFER_MAP.end() != it_offer)
    {
        ...
        if (NULL != pOfferData) // Should never be NULL.
        {
            ...
            {
                const std::string str_price_per_scale(Moneychanger::It()->OT().Exec().FormatAmount(pOfferData->instrument_definition_id,
                                                                               lScale));
            ...
            if (NULL != pTradeList)
            {
                ...
                for (size_t trade_index = 0; trade_index < nTradeDataCount; ++trade_index)
                {
                    ...
                    // -----------------------------------------------------------------------
                    int64_t lOfferID = Moneychanger::It()->OT().Exec().StringToLong(pOfferData->transaction_id);
                    int64_t lTradeID = Moneychanger::It()->OT().Exec().StringToLong(pTradeData->transaction_id);
...
//                  time_t tDate = static_cast<time_t>(Moneychanger::It()->OT().Exec().StringToLong(pTradeData->date));
                    ...
                    std::string   str_price_display = Moneychanger::It()->OT().Exec().FormatAmount(pOfferData->currency_type_id, lPrice);
                    ...
                    std::string   str_amount_display = Moneychanger::It()->OT().Exec().FormatAmount(pOfferData->instrument_definition_id, lQuantity);

                    QString qstrAmountSold = QString::fromStdString(str_amount_display);
                    // -----------------------------------------------------------------------
                    std::string & str_currency_paid   = pTradeData->currency_paid;
                    int64_t       lPayQuantity        = Moneychanger::It()->OT().Exec().StringToLong(str_currency_paid); // Total currency paid

                    if (lPayQuantity < 0)
                        lPayQuantity *= (-1);

                    std::string   str_paid_display    = Moneychanger::It()->OT().Exec().FormatAmount(pOfferData->currency_type_id, lPayQuantity);
                    ...
                    if (0 == lPrice)
                    {
                        if ((lQuantity != 0) && (lPayQuantity != 0) && (lScale != 0))
                        {
                            ...
                            str_price_display = Moneychanger::It()->OT().Exec().FormatAmount(pOfferData->currency_type_id, lPrice);
