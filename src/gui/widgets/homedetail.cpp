...
QString MTHomeDetail::FindAppropriateDepositAccount(opentxs::OTRecord& recordmt)
{
    ...
    if (!qstr_acct_id.isEmpty()) // There IS a default account...
    {
        ...
        str_acct_nym     = opentxs::OTAPI_Wrap::Exec()->GetAccountWallet_NymID      (str_acct_id);
        str_acct_server  = opentxs::OTAPI_Wrap::Exec()->GetAccountWallet_NotaryID   (str_acct_id);
        str_acct_asset   = opentxs::OTAPI_Wrap::Exec()->GetAccountWallet_InstrumentDefinitionID(str_acct_id);
        // -----------------------------------
        str_acct_type    = opentxs::OTAPI_Wrap::Exec()->GetAccountWallet_Type(str_acct_id);
        // -----------------------------------
...
//                // -------------------------------------------
//                if (qstr_current_id.isEmpty() && (opentxs::OTAPI_Wrap::Exec()->GetNymCount() > 0))
//                    qstr_current_id = QString::fromStdString(opentxs::OTAPI_Wrap::Exec()->GetNym_ID(0));
//                // -------------------------------------------
...
//                // -----------------------------------------------
//                const int32_t the_count = opentxs::OTAPI_Wrap::Exec()->GetNymCount();
//                // -----------------------------------------------
...
//                            bFoundDefault = true;
//                        // -----------------------------------------------
//                        OT_name = QString::fromStdString(opentxs::OTAPI_Wrap::Exec()->GetNym_Name(OT_id.toStdString()));
//                        // -----------------------------------------------
...
QWidget * MTHomeDetail::CreateDetailHeaderWidget(opentxs::OTRecord& recordmt, bool bExternal/*=true*/)
{
    ...
    if (recordmt.IsInvoice() || recordmt.IsPaymentPlan() || recordmt.IsNotice() ||
        ((0 == recordmt.GetInstrumentType().compare("chequeReceipt")) &&
         (( recordmt.IsOutgoing() && (opentxs::OTAPI_Wrap::Exec()->StringToLong(recordmt.GetAmount()) > 0)) ||
          (!recordmt.IsOutgoing() && (opentxs::OTAPI_Wrap::Exec()->StringToLong(recordmt.GetAmount()) < 0)))
         ))
    ...
    {
        const int64_t lAmount = opentxs::OTAPI_Wrap::Exec()->StringToAmount(recordmt.GetInstrumentDefinitionID(), recordmt.GetAmount());
...
//          QToolButton *buttonLock  = new QToolButton;
    ...
    long lDate = opentxs::OTAPI_Wrap::Exec()->StringToLong(recordmt.GetDate());
...
//                                             cancelButtonTitle:QString("OK") otherButtonTitles:nil];
    ...
    if (!qstr_NotaryID.isEmpty())
    {
        ...
        QString qstr_name = QString::fromStdString(opentxs::OTAPI_Wrap::Exec()->GetServer_Name(qstr_NotaryID.toStdString()));
    ...
    if (!qstr_AssetTypeID.isEmpty())
    {
        ...
        QString qstr_name = QString::fromStdString(opentxs::OTAPI_Wrap::Exec()->GetAssetType_Name(qstr_AssetTypeID.toStdString()));
...
/*
    if (recordmt.IsPaymentPlan())
    {
        ...
        {
            std::string str_asset_name = opentxs::OTAPI_Wrap::Exec()->GetAssetType_Name(recordmt.GetInstrumentDefinitionID().c_str());
            // ---------------------------------
            ...
            if (recordmt.HasInitialPayment())
            {
                ...
                std::string str_output = opentxs::OTAPI_Wrap::Exec()->FormatAmount(recordmt.GetInstrumentDefinitionID().c_str(),
