...
void BtcGuiTest::on_testButton_clicked()
{
    ...
    {
        opentxs::OTLog::Output(0, "Successfully tested bitcoin functionality.\n");
    }
    else
    {
        opentxs::OTLog::Output(0, "Error testing bitcoin functionality.\nMaybe test environment is not set up properly?");
    }

    /*  deprecated:
    if(!Modules::btcInterface->TestBtcJson())
        opentxs::OTLog::vOutput(0, "Error testing bitcoin integration. Maybe test environment is not set up.\n");
    else
        opentxs::OTLog::vOutput(0, "Bitcoin integration successfully tested.\n");

    if(!Modules::btcInterface->TestBtcJsonEscrowTwoOfTwo())
        opentxs::OTLog::vOutput(0, "Error testing bitcoin escrow functions. Maybe test environment is not set up.\n");
    else
        opentxs::OTLog::vOutput(0, "Bitcoin escrow integration sucessfully tested.\n");
    */
}
