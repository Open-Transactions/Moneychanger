...
void MTPageAcct_Server::initializePage() //virtual
{
    ...
    // -------------------------------------------
    if (qstr_id.isEmpty() && (opentxs::OTAPI_Wrap::Exec()->GetServerCount() > 0))
        qstr_id = QString::fromStdString(opentxs::OTAPI_Wrap::Exec()->GetServer_ID(0));
    // -------------------------------------------
    if (!qstr_id.isEmpty())
        str_name = opentxs::OTAPI_Wrap::Exec()->GetServer_Name(qstr_id.toStdString());
    // -------------------------------------------
...
void MTPageAcct_Server::on_pushButtonManage_clicked()
{
    ...
    // -------------------------------------
    if (bStartingWithNone && (opentxs::OTAPI_Wrap::Exec()->GetServerCount() > 0))
    {
        std::string str_id = opentxs::OTAPI_Wrap::Exec()->GetServer_ID(0);

        if (!str_id.empty())
        {
            std::string str_name = opentxs::OTAPI_Wrap::Exec()->GetServer_Name(str_id);
    ...
    // -------------------------------------
    else if (opentxs::OTAPI_Wrap::Exec()->GetServerCount() < 1)
        SetFieldsBlank();
}
