...
//      int nNumberChecked =
        ...
        if (!qstr_acct_asset.isEmpty())
            qstr_asset_name = QString("   (%1)").arg(
                    QString::fromStdString(opentxs::SwigWrap::Exec()->GetAssetType_Name(str_acct_asset)));
        // -----------------------------------
        ...
        {
            // --------------------------------------
            std::string str_purse = opentxs::SwigWrap::Exec()->LoadPurse(str_acct_server, str_acct_asset, str_acct_nym);

            if (!str_purse.empty())
            {
                int32_t purse_count = opentxs::SwigWrap::Exec()->Purse_Count(str_acct_server, str_acct_asset, str_purse);
                // -------------------------------------------------------
                ui->tableWidget->setRowCount(static_cast<int>(purse_count));
                // -------------------------------------------------------
                for (int ii = 0; ii < purse_count; ii++)
                {
                    std::string cash_token = opentxs::SwigWrap::Exec()->Purse_Peek(str_acct_server, str_acct_asset, str_acct_nym, str_purse);

                    if (!cash_token.empty())
                    {
                        std::string str_amount = opentxs::SwigWrap::Exec()->FormatAmount(str_acct_asset,
                                                                          opentxs::SwigWrap::Exec()->Token_GetDenomination(str_acct_server,
                                                                                                            str_acct_asset,
                                                                                                            cash_token));
                        // ------------------------------------------------------
                        QDateTime qdate_expires     = QDateTime::fromTime_t(opentxs::SwigWrap::Exec()->Token_GetValidTo(str_acct_server, str_acct_asset, cash_token));
                        QString   qstr_token_id     = QString::fromStdString(opentxs::SwigWrap::Exec()->Token_GetID(str_acct_server, str_acct_asset, cash_token));
                        QString   qstr_denomination = QString::fromStdString(str_amount);
                        QString   qstr_series       = QString("%1").arg(opentxs::SwigWrap::Exec()->Token_GetSeries(str_acct_server, str_acct_asset, cash_token));
...
//                      ui->tableWidget->setCellWidget ( ii, 0, pCheckbox );
                    ...
                    str_purse = opentxs::SwigWrap::Exec()->Purse_Pop(str_acct_server, str_acct_asset, str_acct_nym, str_purse);
                } // for
                // -------------------------------------------------------
            } // if purse not empty.
        } // if cash balance > 0.
...
void MTCashPurse::on_pushButtonWithdraw_clicked()
{
    ...
    if (!bSent)
    {
        const std::string str_server = opentxs::SwigWrap::Exec()->GetAccountWallet_NotaryID(accountID);
        const std::string str_nym    = opentxs::SwigWrap::Exec()->GetAccountWallet_NymID   (accountID);
...
void MTCashPurse::on_pushButtonExport_clicked()
{
    ...
    if (qstrSelectedIndices.isEmpty())
        return;
    // ------------------------------------
    std::string str_amount = opentxs::SwigWrap::Exec()->FormatAmount(str_acct_asset, lAmount);
    // ------------------------------------
    ...
    for (int ii = 0; ii < ui->tableWidget->rowCount(); ii++)
    {
        ...
        if (NULL != pWidget)
        {
            ...
            if ( (NULL != pCheckbox) && (Qt::Checked == pCheckbox->checkState()))
            {
                ...
                if (NULL != pValueLabel)
                {
                    int64_t lTokenAmount = opentxs::SwigWrap::Exec()->StringToAmount(m_qstrInstrumentDefinitionID.toStdString(),
...
//                  selectedIndices.append(pIDLabel->text());
    ...
    if (nNumberSelected > 0)
    {
        ...
        if (m_qstrInstrumentDefinitionID.isEmpty())
            ...
            qstr_amount = QString::fromStdString(opentxs::SwigWrap::Exec()->FormatAmount(m_qstrInstrumentDefinitionID.toStdString(), lAmount));
        // ---------------------------------------
