...
QWidget(parent)
{
    ...
    already_init = false;
    refreshing = false;
    account_manager_proccessing_dataChanged = false;
    
    //"Add account" dialog
    accountmanager_addaccount_dialog_already_init = false;
    accountmanager_addaccount_dialog_advanced_showing = false;
    
    //"Remove account" dialog
    account_manager_removeaccount_dialog_already_init = false;
...
void AccountManagerWindow::dialog(){
    ...
    if(!already_init){
        ...
        /** window properties **/
        //Set window title
        account_manager_dialog->setWindowTitle("Account Manager | Moneychanger");
        
        //Set window on top
        //account_manager_dialog->setWindowFlags(Qt::WindowStaysOnTopHint);
        
        /** layout and content **/
        //Grid layout
        account_manager_gridlayout = new QGridLayout(0);
        account_manager_dialog->setLayout(account_manager_gridlayout);
        
        /* First Row in account manager Grid */
        //Label (header)
        account_manager_label = new QLabel("<h3>Account Manager</h3>", 0);
        account_manager_label->setAlignment(Qt::AlignRight);
        account_manager_gridlayout->addWidget(account_manager_label, 0,0, 1,2, Qt::AlignRight);
        ...
        account_manager_holder = new QWidget(0);
        account_manager_hbox = new QHBoxLayout(0);
        account_manager_holder->setLayout(account_manager_hbox);
        account_manager_gridlayout->addWidget(account_manager_holder);
        
        //Table View (backend and visual init)
        account_manager_tableview_itemmodel = new QStandardItemModel(0,3,0);
        account_manager_tableview_itemmodel->setHorizontalHeaderItem(0, new QStandardItem(QString("Account Display Name")));
        account_manager_tableview_itemmodel->setHorizontalHeaderItem(1, new QStandardItem(QString("Account ID")));
        account_manager_tableview_itemmodel->setHorizontalHeaderItem(2, new QStandardItem(QString("Default")));
        
        //Connect tableviews' backend "dataChanged" signal to a re-action.
        connect(account_manager_tableview_itemmodel, SIGNAL(dataChanged(QModelIndex,QModelIndex)), this, SLOT(dataChanged_slot(QModelIndex,QModelIndex)));
        
        account_manager_tableview = new QTableView(0);
        account_manager_tableview->setSelectionMode(QAbstractItemView::SingleSelection);
        account_manager_tableview->setModel(account_manager_tableview_itemmodel);
        account_manager_tableview->setColumnWidth(0, 175);
        account_manager_tableview->setColumnWidth(1, 150);
        account_manager_tableview->setColumnWidth(2, 75);
        account_manager_gridlayout->addWidget(account_manager_tableview, 1,0, 1,1);
        
        
        /** Second column in account manager grid (right side) **/
        //Vertical box (contains: add/remove buttons)
        account_manager_addremove_btngroup_holder = new QWidget(0);
        account_manager_addremove_btngroup_vbox = new QVBoxLayout(0);
        account_manager_addremove_btngroup_holder->setLayout(account_manager_addremove_btngroup_vbox);
        account_manager_addremove_btngroup_vbox->setAlignment(Qt::AlignTop);
        account_manager_gridlayout->addWidget(account_manager_addremove_btngroup_holder, 1,1, 1,1);
        
        //"Add account" button
        account_manager_addremove_btngroup_addbtn = new QPushButton("Add Account", 0);
        account_manager_addremove_btngroup_addbtn->setStyleSheet("QPushButton{padding:0.5em;}");
        account_manager_addremove_btngroup_vbox->addWidget(account_manager_addremove_btngroup_addbtn, Qt::AlignTop);
        //Connect the add account button with a re-action to it being "clicked"
        connect(account_manager_addremove_btngroup_addbtn, SIGNAL(clicked()), this, SLOT(addaccount_slot()));
        
        //"Remove account" button
        account_manager_addremove_btngroup_removebtn = new QPushButton("Remove Account", 0);
        account_manager_addremove_btngroup_removebtn->setStyleSheet("QPushButton{padding:0.5em;}");
        account_manager_addremove_btngroup_vbox->addWidget(account_manager_addremove_btngroup_removebtn, Qt::AlignTop);
        //Connect the remove account button with a re-action to it being "clicked"
        connect(account_manager_addremove_btngroup_removebtn, SIGNAL(clicked()), this, SLOT(removeaccount_slot()));
        
        /** Third column (Most revent error) **/
        //Label
        account_manager_most_recent_erorr = new QLabel("");
        account_manager_gridlayout->addWidget(account_manager_most_recent_erorr, 2,0, 1,2, Qt::AlignHCenter);
        /** Flag as init **/
        already_init = true;
    }
    /** ***
     ** Resize & Show
     **/
    account_manager_dialog->resize(600, 300);
    account_manager_dialog->show();
    ...
    account_manager_tableview_itemmodel->removeRows(0, account_manager_tableview_itemmodel->rowCount());
    ...
    for(int a = 0; a < total_accounts; a++){
        ...
        account_manager_tableview_itemmodel->setItem(row_index, 0, col_one);
        account_manager_tableview_itemmodel->setItem(row_index, 1, col_two);
        account_manager_tableview_itemmodel->setItem(row_index, 2, col_three);
...
void AccountManagerWindow::addaccount_slot(){
    ...
    if(!accountmanager_addaccount_dialog_already_init){
        ...
        account_manager_addaccount_dialog->setModal(1);
        
        //Gridlayout
        account_manager_addaccount_gridlayout = new QGridLayout(0);
        account_manager_addaccount_dialog->setLayout(account_manager_addaccount_gridlayout);
        
        //Label (header)
        account_manager_addaccount_header = new QLabel("<h2>Add Account</h2>",0);
        account_manager_addaccount_gridlayout->addWidget(account_manager_addaccount_header, 0,0, 1,1, Qt::AlignRight);
        
        //Label (Show Advanced Option(s)) (Also a button/connection)
        account_manager_addaccount_subheader_toggleadvanced_options_label = new QLabel("<a href='#'>Advanced Option(s)</a>", 0);
        account_manager_addaccount_gridlayout->addWidget(account_manager_addaccount_subheader_toggleadvanced_options_label, 1,0, 1,1, Qt::AlignRight);
        //Connect with a re-action
        connect(account_manager_addaccount_subheader_toggleadvanced_options_label, SIGNAL(linkActivated(QString)), this, SLOT(dialog_showadvanced_slot(QString)));
        
        
        //Label (instructions)
        account_manager_addaccount_subheader_instructions = new QLabel("Below are some options that will help determine how your Account will be added.");
        account_manager_addaccount_subheader_instructions->setWordWrap(1);
        account_manager_addaccount_subheader_instructions->setStyleSheet("QLabel{padding:0.5em;}");
        ...
        account_manager_addaccount_create_account_btn->setStyleSheet("QPushButton{padding:1em;}");
        account_manager_addaccount_gridlayout->addWidget(account_manager_addaccount_create_account_btn, 5,0, 1,1, Qt::AlignHCenter);
        //Connect create account button with a re-action;
        connect(account_manager_addaccount_create_account_btn, SIGNAL(clicked()), this, SLOT(dialog_createaccount_slot()));
        
        /** Flag as already init **/
        accountmanager_addaccount_dialog_already_init = true;
    }
    //Resize
    account_manager_addaccount_dialog->resize(400, 290);
    //Show
...
void AccountManagerWindow::removeaccount_slot(){
    ...
    if(!account_manager_removeaccount_dialog_already_init){
        ...
        //Grid layout
        account_manager_removeaccount_gridlayout = new QGridLayout(0);
        account_manager_removeaccount_dialog->setLayout(account_manager_removeaccount_gridlayout);
    }
    account_manager_removeaccount_dialog->resize(400,250);
    account_manager_removeaccount_dialog->show();
}
...
void AccountManagerWindow::dataChanged_slot(QModelIndex topLeft, QModelIndex bottomRight){
    ...
    if(!refreshing && account_manager_proccessing_dataChanged == 0){
        /** Flag Proccessing dataChanged **/
        account_manager_proccessing_dataChanged = 1;
        
        /** Proccess the "Display Name" column **/
        if(topLeft.column() == 0){
            //Get the value (as std::string) of the account id
            QStandardItem * account_id_item = account_manager_tableview_itemmodel->item(topLeft.row(), 1);
            ...
            if(setName_successfull == true){
                ...
                account_manager_most_recent_erorr->setText("<span style='color:#A80000'><b>Renaming that account failed. (Error Code: 100)</b></span>");
            }
        }
        ...
        if(topLeft.column() == 2){
            ...
            int total_checkboxes = account_manager_tableview_itemmodel->rowCount();
            ...
            for(int a = 0; a < total_checkboxes; a++){
                ...
                if(a != topLeft.row()){
                    ...
                    //Get checkbox item
                    QStandardItem * checkbox_model = account_manager_tableview_itemmodel->item(a, 2);
                    
                    //Update the checkbox item at the backend.
                    checkbox_model->setCheckState(Qt::Unchecked);
                    
                    //Update the checkbox item visually.
                    account_manager_tableview_itemmodel->setItem(a, 2, checkbox_model);
                    ...
                    QStandardItem * account_id = account_manager_tableview_itemmodel->item(a, 1);
        ...
        account_manager_proccessing_dataChanged = 0;
    }
}
...
void AccountManagerWindow::dialog_showadvanced_slot(QString link_href){
    ...
    if(!accountmanager_addaccount_dialog_advanced_showing){
        //Show advanced options.
        //Show the Bits option
        
    }else if(accountmanager_addaccount_dialog_advanced_showing){
...
void AccountManagerWindow::request_remove_account_slot(){
    ...
    QModelIndexList selected_indexes = account_manager_tableview->selectionModel()->selectedIndexes();
    ...
    for(int a = 0; a < selected_indexes.size(); a++){
        ...
        //Get account id
        QModelIndex account_id_modelindex = account_manager_tableview_itemmodel->index(selected_row, 1, QModelIndex());
